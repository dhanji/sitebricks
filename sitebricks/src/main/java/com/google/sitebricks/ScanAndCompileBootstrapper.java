...
class ScanAndCompileBootstrapper implements Bootstrapper {
    ...
    @Inject
    public ScanAndCompileBootstrapper(PageBook pageBook, @Bricks List<Package> packages,
            ResourcesService resourcesService, WidgetRegistry registry, SystemMetrics metrics, Compilers compilers) {
    ...
    public void start() {
        ...
        for (Package pkg : packages) {
            ...
            set.addAll(Classes.matching(
                    annotatedWith(At.class).or(annotatedWith(EmbedAs.class)).or(annotatedWith(With.class))
                            .or(annotatedWith(Show.class))).in(pkg));
        }
    ...
    private void collectBindings(List<SitebricksModule.LinkingBinder> bindings, Set<PageBook.Page> pagesToCompile) {
    ...
    private Set<PageBook.Page> scanPagesToCompile(Set<Class<?>> set) {
        ...
        for (Class<?> pageClass : set) {
            ...
            if (pageClass.isAnnotationPresent(Show.class)) {
                ...
                templates.add(new Templates.Descriptor(pageClass, pageClass.getAnnotation(Show.class).value()));
            }
        }
