...
public class DefaultPageBook implements PageBook {
            ...
            if (method.isAnnotationPresent(At.class)) {
                ...
                if (!subpath.startsWith("/") || subpath.isEmpty() || subpath.length() == 1) {
                    throw new IllegalArgumentException(String.format(
                            "Subpath At(\"%s\") on %s.%s() must begin with a \"/\" and must not be empty", subpath,
                            pageClass.getName(), method.getName()));
                }
    ...
    private PageTuple at(String uri, Class<?> clazz, boolean headless) {
        ...
        final PageTuple pageTuple = new PageTuple(uri, new PathMatcherChain(uri), clazz, injector, headless, false);
    ...
    public static class PageTuple implements Page {
        ...
        private static final Key<Map<String, Class<? extends Annotation>>> HTTP_METHODS_KEY = Key.get(
                new TypeLiteral<Map<String, Class<? extends Annotation>>>() {
                }, Bricks.class);
        ...
        private Multimap<String, Action> reflectAndCache(String uri, Map<String, Class<? extends Annotation>> methodMap) {
            String tail = "";
    ...
    private static class MethodTuple implements Action {
        ...
        private List<Object> reflect(Method method) {
            ...
            for (int i = 0; i < annotationsGrid.length; i++) {
                ...
                if (!namedFound) {
                    ...
                    Key<?> key = (null != bindingAnnotation) ? Key.get(argType, bindingAnnotation) : Key.get(argType);
        ...
        private static Object call(Object page, final Method method, Object[] args) {
            try {
                ...
                throw new EventDispatchException("Could not access event method (appears to be a security problem): "
                        + method, e);
                ...
                throw new EventDispatchException(String.format(
                        ...
                        cause.getMessage(), method, stackTrace[0]), e);
            }
        }
        ...
        private Map<String, String> discoverNegotiates(Method method, Injector injector) {
            ...
            Map<String, Class<? extends Annotation>> negotiationsMap = injector.getInstance(Key.get(
                    new TypeLiteral<Map<String, Class<? extends Annotation>>>() {
                    }, Negotiation.class));
    ...
    static String readAnnotationValue(Annotation annotation) {
        try {
            ...
            throw new IllegalStateException("Encountered a configured annotation that " + "could not be read."
                    + annotation, e);
        } catch (IllegalAccessException e) {
            throw new IllegalStateException("Encountered a configured annotation that " + "could not be read."
                    + annotation, e);
