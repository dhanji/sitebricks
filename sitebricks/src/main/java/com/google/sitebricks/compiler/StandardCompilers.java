...
class StandardCompilers implements Compilers {
    ...
    public StandardCompilers(WidgetRegistry registry, PageBook pageBook, SystemMetrics metrics,
            ...
            VelocityEngineProvider velocityEngineProvider) {
    ...
    public Renderable compileHtml(Class<?> page, String template) {
        return new HtmlTemplateCompiler(page, new MvelEvaluatorCompiler(page), registry, pageBook, metrics)
                .compile(template);
    }

    @Override
    public Renderable compileFlat(Class<?> page, String template) {
        return new FlatTemplateCompiler(page, new MvelEvaluatorCompiler(page), metrics, registry).compile(template);
    }
